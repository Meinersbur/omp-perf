...
BENCHMARK_TEMPLATE(benchmark_atax_host, 32)->Unit(benchmark::kMicrosecond);
...
template<int P>
static //__attribute__((noinline))
void kernel_atax_target(int m, int n,
		 DATA_TYPE POLYBENCH_2D(A,M,N,m,n),
		 DATA_TYPE POLYBENCH_1D(x,N,n),
		 DATA_TYPE POLYBENCH_1D(y,N,n),
		 DATA_TYPE POLYBENCH_1D(tmp,M,m)) {
  ...
  #pragma omp target teams distribute parallel for// map(to:A[0:M][0:N]) map(to:x[0:N]) map(from:y[0:N]) map(to:tmp[0:M])
...
template<int P>
static void benchmark_atax_target_inner(
     int m, int n,
		 DATA_TYPE POLYBENCH_2D(A,M,N,m,n),
		 DATA_TYPE POLYBENCH_1D(x,N,n),
		 DATA_TYPE POLYBENCH_1D(y,N,n),
		 DATA_TYPE POLYBENCH_1D(tmp,M,m), benchmark::State& state) {

  #pragma omp target data map(to:A[0:M][0:N]) map(to:x[0:N]) map(from:y[0:N]) map(alloc:tmp[0:M])
  {
    for (auto _ : state) {
    /* Run kernel. */
    kernel_atax_target<P>(m, n,
          A,
          x,
          y,
          tmp);
...
template <int P>
static void benchmark_atax_target(benchmark::State& state) {
  /* Retrieve problem size. */
  int m = M;
  int n = N;
...
benchmark_atax_target_inner<P>(m, n,
	       POLYBENCH_ARRAY(A),
	       POLYBENCH_ARRAY(x),
	       POLYBENCH_ARRAY(y),
	       POLYBENCH_ARRAY(tmp), state);
